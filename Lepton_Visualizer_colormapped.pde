...
//  checkFrame();
  ...
  for (int y = 0; y < ROWS; y++) {
    for (int x = 0; x <COLS; x++) {
      ...
      pixel = (int) map(pixel, MIN_TEMP, MAX_TEMP, 1, 256);
//      pixel >>= 4;
      pixel &= 0xff;
      color c = color(colormap[3*pixel], colormap[3*pixel+1], colormap[3*pixel+2]);
      //color c = color(pixel, 100, 100);
